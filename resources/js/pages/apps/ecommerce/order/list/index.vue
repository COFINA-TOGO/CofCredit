          ...
          <template v-for="(data, id) in widgetData" :key="id">
            ...
            <VDivider v-if="$vuetify.display.mdAndUp ? id !== widgetData.length - 1
              : $vuetify.display.smAndUp ? id % 2 === 0
                : false" vertical inset length="55" />
          </template>
        </VRow>
      </VCardText>
    </VCard>
    ...
    <VCard>
      ...
      <VCardText>
        <div class="d-flex justify-sm-space-between justify-start flex-wrap gap-4">
          <VTextField v-model="searchQuery" density="compact" placeholder="Serach Order"
            style=" max-inline-size: 200px; min-inline-size: 200px;" />

          <div class="d-flex gap-x-4 align-center">
            <AppSelect v-model="itemsPerPage" style="min-inline-size: 6.25rem;" :items="[5, 10, 20, 50, 100]" />
            <VBtn variant="tonal" color="secondary" prepend-icon="tabler-screen-share" text="Export"
      ...
      <VDataTableServer v-model:items-per-page="itemsPerPage" v-model:page="page" :headers="headers" :items="orders"
        :items-length="totalOrder" show-select class="text-no-wrap" @update:options="updateOptions">
        <!-- Order ID -->
        ...
        <template #item.customers="{ item }">
          <div class="d-flex align-center">
            <VAvatar size="38" :color="!item.avatar.length ? 'primary' : ''"
              :variant="!item.avatar.length ? 'tonal' : undefined" :rounded="1" class="me-2">
            ...
            <div class="d-flex flex-column">
              <div class="text-body-1 font-weight-medium">
                <RouterLink :to="{ name: 'pages-user-profile-tab', params: { tab: 'profile' } }" class="text-link">
                  {{ item.customer }}
                </RouterLink>
              </div>
        ...
        <template #item.payment="{ item }">
          <li :class="`text-${resolvePaymentStatus(item.payment)?.color}`" class="font-weight-medium">
        ...
        <template #item.method="{ item }">
          <div class="d-flex align-start gap-x-2">
            <VImg :src="item.method === 'mastercard' ? mastercard : paypal" height="22" max-width="22" min-width="22" />
            <div>
              <VIcon icon="tabler-dots" class="me-2" />
