	...
	<div>
		...
		<VCard class="mb-6">
			<VCardText>
				<div class="d-flex flex-wrap gap-4 mx-5">
					...
					<div class="d-flex gap-4 flex-wrap align-center">
						...
						<VBtn v-if="$can('create', 'basic-cat')" color="primary" prepend-icon="tabler-plus"
							:to="{ name: 'cat-add' }">
							Ajouter
						</VBtn>
			...
			<VDataTableServer v-model:items-per-page="itemsPerPage" v-model:page="page" :headers="headers"
				:items="catList" :items-length="totalCAT" class="text-no-wrap" @update:options="updateOptions">
				...
				<template #item.status="{ item }">
					<VChip label :color="item.status.color">
						<VTooltip v-if="item.validation_comment && !unblock_comment" activator="parent"
							transition="scroll-x-transition" location="start">Raison: {{ item.validation_comment }}
						</VTooltip>
							...
							<VMenu activator="parent">
								<VList>
									...
									<VListItem v-if="$can('download', 'basic-cat')"
										@click="downloadFile(`/api/cat/download/${item.id}`, `CAT-${item.contract.verbal_trial.committee_id}.pdf`)">
										<template #prepend>
