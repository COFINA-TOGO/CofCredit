					...
					<VList density=" compact">
						...
						<VListItem v-if="item.observations.length == 0">
							<VChip label
								...
								<VTooltip v-if="item.status" activator="parent" transition="scroll-x-transition"
									location="start">
				...
				<template #item.actions="{ item }">
					...
					<span
						...
						<IconBtn v-if="$can('create', 'cat') && item.status == 'validated'"
							...
							<VTooltip activator="parent" transition="scroll-x-transition" location="end">Créer le CAT
							</VTooltip>
