...
const tableData = [
	...
	{ "title": "Prime d'assurance", "value": "Selon la grille de l'assureur"},
]
				...
				<VCard>
					...
					<VCardText class="d-flex flex-wrap justify-space-between flex-column flex-sm-row print-row text-lg">
						...
						<VCol v-if="notification.status == 'rejected' && notification.status_observation">
							<VAlert color="warning">
								Motif du refus par {{ notification.creator.full_name }}: {{
									notification.status_observation }}
							</VAlert>
						</VCol>
						<VCol
							v-if="notification.head_credit_validation == 'rejected' && notification.head_credit_observation">
							<VAlert color="warning">
					...
					<VCardText class="d-flex flex-wrap justify-space-between flex-column flex-sm-row print-row text-lg">
						...
						<VCol cols="12">
							<p>
							<ul>
								<li v-for="(item, index) in notification.guarantees" :key="index"
									style="font-size: 20px">
									{{ item.type_of_guarantee.name }} de {{
										String(item.value).replace(/\B(?=(\d{3})+(?!\d))/g, ' ') }} F
									CFA : {{ item.comment }}
								</li>
							</ul>
							</p>
