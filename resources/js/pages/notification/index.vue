...
const apiChangeStatus = async id => {
  try {
    ...
    showSnackbar('success', `Notification ${statusMessage} avec succès`)
    ...
    <VCard class="mb-6">
      ...
      >
        ...
        <template #item.head_credit_validation="{ item }">
          ...
          >
            <VTooltip
              v-if="item.head_credit_observation"
              activator="parent"
              transition="scroll-x-transition"
              location="start"
            >
              Raison: {{ item.head_credit_observation }}
            </VTooltip>
        ...
        <template #item.actions="{ item }">
          ...
          <VDivider />
          <IconBtn
            v-if="$can('reject', 'notification') && item.head_credit_validation != 'rejected'"
          ...
          <span v-if="item.head_credit_validation == 'waiting'">
            <IconBtn
              v-if="$can('validate', 'notification')"
