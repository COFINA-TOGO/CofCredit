		...
		<VCard title="Filtres" class="mb-6">
			...
			<VDataTableServer v-model:items-per-page="itemsPerPage" v-model:page="page" :headers="headers"
				...
				<template #item.status="{ item }">
					<VChip label
						:color="{ 'validated': 'success', 'rejected': 'error', 'waiting': 'warning' }[item.status]">
						...
						({{ {
							'credit_admin': 'Admin Crédit', 'head_credit': "Head Crédit", 'md': 'MD'
				...
				<template #item.actions="{ item }">
					...
					<div v-if="$can('create', 'notification') && item.status == 'validated'">
						...
						<span class="full-width-icon" col="12" v-if="item.status == 'validated'">
							<IconBtn v-if="$can('create', 'notification')"
								:to="{ name: 'notification-add', query: { id: item.id } }">
