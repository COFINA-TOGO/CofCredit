...
class VerbalTrialController extends Controller
{
     ...
     * @queryParam  applicant_name                      string  Filtrer par nom du demandeur                                    No-example
     ...
     * @queryParam  due_amount                          float   Filtrer par échéance                                            No-example
     ...
     * @queryParam  line_review_bonus                   float   Filtrer par prime de révision de ligne                          No-example
     *
        ...
        if (($authorisation = Gate::inspect('viewAny', VerbalTrial::class))->allowed()) {
            ...
            if ($search = $request->search) {
                $verbalTrialList
                    ...
                    ->orWhere('line_review_bonus', 'LIKE', "%$search%")
                ;
            }

            foreach (["committee_id", "committee_date", "civility", "applicant_name", "account_number", "activity", "purpose_of_financing", "type_of_credit_id", "amount", "duration", "periodicity", "taf", "due_amount", "administrative_fees_percentage", "insurance_premium", "line_review_bonus"] as $filter) {
                if (isset($request[$filter]) && $request[$filter]) {
                    $verbalTrialList->where($filter, $request[$filter]);
                }
            }

            foreach (["with_type_of_credit" => "type_of_credit", "with_type_of_applicant" => "type_of_credit.type_of_applicant", "with_guarantees" => "guarantees"] as $key => $value) {
     ...
     * @bodyParam  applicant_name                      string   Le nom du demandeur                                    Example: Cesar Endure
     ...
     * @bodyParam  due_amount                          float    L'échéance                                             Example: 500000
     ...
     * @bodyParam  line_review_bonus                   float    La prime de révision de ligne                          Example: 1
     *
     * @response 200
     */
    public function store(Request $request)
    {
        if (($authorisation = Gate::inspect('create', VerbalTrial::class))->allowed()) {
            ...
            $validator = Validator::make($requestData, [
                ...
                'line_review_bonus' => 'required|numeric',
                "guarantees" => "array",
     ...
     * @bodyParam  applicant_name                      string   Le nom du demandeur                                    Example: Cesarette Endure
     * @bodyParam  account_number                      string   Le numéro de compte                                    Example: 012345678901
